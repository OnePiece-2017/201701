...
public class NativeQuery<E> extends Query<EntityManager, E> {
	...
	private void initResultCount() {
		if (resultCount == null) {
			...
			resultCount = query == null ? null : Long.valueOf(query.getSingleResult().toString());
		}
	}

	/**
	 * The refresh method will cause the result to be cleared. The next access to the result set will cause the query to be executed.
	 * 
	 * This method <b>does not</b> cause the ejbql or restrictions to reread. If you want to update the ejbql or restrictions you must call {@link #setEjbql(String)} or {@link #setRestrictions(List)}
	 */
	@Override
	public void refresh() {
		super.refresh();
	...
	protected javax.persistence.Query createQuery() {
		...
		javax.persistence.Query query = getEntityManager().createNativeQuery(getRenderedEjbql());
	...
	protected javax.persistence.Query createCountQuery() {
		...
		javax.persistence.Query query = getEntityManager().createNativeQuery("select count(1) from (" + getEjbql() + ") as ejbql");
		return query;
	}
