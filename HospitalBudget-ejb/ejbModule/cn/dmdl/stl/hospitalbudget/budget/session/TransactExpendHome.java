...
@Name("transactExpendHome")
public class TransactExpendHome extends CriterionEntityHome<Object> {
	...
	private void updateProjectTaskOrderId(int oldTaskOrderId, int newTaskOrderId) {
		List<NormalExpendBudgetOrderInfo> oldNormalExpendBudgetOrderInfoList = getEntityManager().createQuery("select normalExpendBudgetOrderInfo from NormalExpendBudgetOrderInfo normalExpendBudgetOrderInfo where normalExpendBudgetOrderInfo.taskOrderId = " + oldTaskOrderId).getResultList();
		if (oldNormalExpendBudgetOrderInfoList != null && oldNormalExpendBudgetOrderInfoList.size() > 0) {
			joinTransaction();
			for (NormalExpendBudgetOrderInfo normalExpendBudgetOrderInfo : oldNormalExpendBudgetOrderInfoList) {
				normalExpendBudgetOrderInfo.setTaskOrderId(newTaskOrderId);// 跟踪订单信息
				getEntityManager().merge(normalExpendBudgetOrderInfo);
			}
