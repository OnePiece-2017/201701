...
@Name("incomeDraftHome")
public class IncomeDraftHome extends CriterionEntityHome<Object>{
	...
	public JSONArray pullNewGenericProject(String args) {
		...
		dataSql.append(" and projectType = 1");// 收入预算
	...
	public JSONArray pullOldProject(String args, String projectIdField, String projectTable, String projectCompilerTable) {
		...
		if (args != null && !"".equals(args)) {
			...
			if (argsJson != null && argsJson.has("budgetYear")) {
				List<Object[]> list = commonTool.selectIntermediate("ys_income_draft", new String[] { projectIdField, "project_amount", "project_source", "formula_remark", "attachment", "status" ,"project_nature"}, "status in (0, 1, 2, 3, 4)" + " and draft_type = " + HospitalConstant.DRAFT_TYPE_DRAFT + " and year = " + argsJson.get("budgetYear") + " and insert_user = " + sessionToken.getUserInfoId() + " and " + projectIdField + " in (select the_id from " + projectTable + " where project_type = 2 and top_level_project_id in (select project_id from " + projectCompilerTable + " where user_info_id = " + sessionToken.getUserInfoId() + "))");
	...
	public boolean buildDataBySection(int buildDataAction, JSONArray projectArray, String projectIdField, String projectType) {
		...
		try {
			for (int i = 0; i < projectArray.size(); i++) {
				...
				List<Object[]> list = commonTool.selectIntermediate("ys_income_draft", new String[] { "ys_income_draft_id" }, "`year` = " + data.get("budgetYear") + " and " + projectIdField + " = " + data.get("projectId") + " and insert_user = " + sessionToken.getUserInfoId());
				...
				Object incomeDraftId = null;// 编制表主键id
				if (list.size() > 0) {
